...
### 🎯 检测识别
- **目标检测**：基于 YOLO 模型，识别 11 类目标（安全帽、人员、机械、车辆、锥形桶、电线杆等）
- **违规判定**：六大违规规则引擎，实时检测 PPE 穿戴、机械/车辆近身、管控区闯入等违规行为
...
| `/video-detect` | VideoDetect.vue | 视频上传与检测 |
...
│   │   │   └── ws.py          # WebSocket
│   │   ├── core/              # 核心检测引擎
│   │   │   ├── detector.py    # YOLO 模型推理
│   │   │   ├── danger_rules.py # 危险判定规则
│   │   │   └── annotator.py   # 标注绘制
